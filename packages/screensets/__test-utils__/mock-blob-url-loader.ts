 ...
 */
// @cpt-begin:cpt-frontx-dod-mfe-isolation-blob-core:p1:inst-mock-blob-loader-state
const blobContentMap = new Map<number, string>();
let blobIdCounter = 0;
// @cpt-end:cpt-frontx-dod-mfe-isolation-blob-core:p1:inst-mock-blob-loader-state
 ...
 */
// @cpt-begin:cpt-frontx-dod-mfe-isolation-blob-core:p1:inst-mock-blob-loader-setup
...
// @cpt-end:cpt-frontx-dod-mfe-isolation-blob-core:p1:inst-mock-blob-loader-setup
 ...
 */
// @cpt-begin:cpt-frontx-dod-mfe-isolation-blob-core:p1:inst-mock-blob-loader-factories
...
export function createMinifiedRemoteEntrySource(
  exposedModules: Record<string, string>
): string {
  const entries = Object.entries(exposedModules)
    .map(
      ([key, chunk]) =>
        `"${key}":()=>(E([],!1,"${key}"),w("./${chunk}").then(e=>e))`
    )
    .join(',');
...
export function createSharedDepSource(bareImports?: string[]): string {
  const imports = bareImports?.map(
    (name) => `import __ext_${name.replaceAll(/\W/g, '_')} from "${name}";`
  ).join('\n') ?? '';
...
// @cpt-end:cpt-frontx-dod-mfe-isolation-blob-core:p1:inst-mock-blob-loader-factories
